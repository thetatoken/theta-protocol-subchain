...
func NewStakeRewardDistributionTxExecutor(state *slst.LedgerState) *StakeRewardDistributionTxExecutor {
	return &StakeRewardDistributionTxExecutor{
		state: state,
	}
}

func (exec *StakeRewardDistributionTxExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
...
func (exec *StakeRewardDistributionTxExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.StakeRewardDistributionTx)
	...
	stakeHolderAddress := tx.Holder.Address
	//if tx.Purpose == score.StakeForGuardian || tx.Purpose == score.StakeForEliteEdgeNode {
	srdsr := slst.NewStakeRewardDistributionRuleSet(view)
	...
	if splitBasisPoint == 0 { // considered as removal
		...
		rd, err := score.NewRewardDistribution(stakeHolderAddress, tx.Beneficiary.Address, splitBasisPoint)
...
func (exec *StakeRewardDistributionTxExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := transaction.(*types.StakeRewardDistributionTx)
	return &score.TxInfo{
