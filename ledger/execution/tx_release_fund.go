...
func NewReleaseFundTxExecutor(state *slst.LedgerState) *ReleaseFundTxExecutor {
	return &ReleaseFundTxExecutor{
		state: state,
	}
}

func (exec *ReleaseFundTxExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
...
func (exec *ReleaseFundTxExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.ReleaseFundTx)
...
func (exec *ReleaseFundTxExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := transaction.(*types.ReleaseFundTx)
	return &score.TxInfo{
