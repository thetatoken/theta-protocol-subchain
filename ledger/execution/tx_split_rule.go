...
func NewSplitRuleTxExecutor(state *slst.LedgerState) *SplitRuleTxExecutor {
	return &SplitRuleTxExecutor{
		state: state,
	}
}

func (exec *SplitRuleTxExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
...
func (exec *SplitRuleTxExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.SplitRuleTx)
...
func (exec *SplitRuleTxExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := transaction.(*types.SplitRuleTx)
	return &score.TxInfo{
