...
func NewSlashTxExecutor(consensus score.ConsensusEngine, valMgr score.ValidatorManager) *SlashTxExecutor {
...
func (exec *SlashTxExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
...
func (exec *SlashTxExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.SlashTx)
...
func (exec *SlashTxExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := transaction.(*types.SlashTx)
	return &score.TxInfo{
