...
// func RetrievePools(ledger score.Ledger, chain *sbc.Chain, db database.Database, blockHeight uint64, guardianVotes *score.AggregatedVotes,
// 	eliteEdgeNodeVotes *score.AggregatedEENVotes) (guardianPool *score.GuardianCandidatePool, eliteEdgeNodePool score.EliteEdgeNodePool) {
// 	guardianPool = nil
...
// 				logger.Panic(err)
// 			}
// 			storeView := slst.NewStoreView(guradianVoteBlock.Height, guradianVoteBlock.StateHash, db)
...
// 					eliteEdgeNodePool = slst.NewEliteEdgeNodePool(storeView, true)
// 				} else {
...
//func RetrievePools(ledger score.Ledger, chain *sbc.Chain, db database.Database, blockHeight uint64, guardianVotes *score.AggregatedVotes) (guardianPool *score.GuardianCandidatePool) {
//	guardianPool = nil
//
//	if blockHeight < common.HeightEnableTheta2 {
//		guardianPool = nil
...
// func CalculateReward(ledger score.Ledger, view *slst.StoreView, validatorSet *score.ValidatorSet,
// 	guardianVotes *score.AggregatedVotes, guardianPool *score.GuardianCandidatePool,
// 	eliteEdgeNodeVotes *score.AggregatedEENVotes, eliteEdgeNodePool score.EliteEdgeNodePool) map[string]types.Coins {
...
// CalculateReward calculates the block reward for each account
func CalculateReward(ledger score.Ledger, view *slst.StoreView, validatorSet *score.ValidatorSet) map[string]types.Coins {
	accountReward := map[string]types.Coins{}
	blockHeight := view.Height() + 1 // view points to the parent block
	if blockHeight < common.HeightEnableValidatorReward {
		grantValidatorsWithZeroReward(validatorSet, &accountReward)
...
func grantValidatorsWithZeroReward(validatorSet *score.ValidatorSet, accountReward *map[string]types.Coins) {
...
func grantValidatorReward(ledger score.Ledger, view *slst.StoreView, validatorSet *score.ValidatorSet, accountReward *map[string]types.Coins, blockHeight uint64) {
...
func grantValidatorAndGuardianReward(ledger score.Ledger, view *slst.StoreView, validatorSet *score.ValidatorSet,
	accountReward *map[string]types.Coins, blockHeight uint64) {
	if !common.IsCheckPointHeight(blockHeight) {
		return
	}
	...
	effectiveStakes := [][]*score.Stake{}          // For compatiblity with old sampling algorithm, stakes from the same staker are grouped together
	...
	for _, v := range validatorSet.Validators() {
		...
		for _, stake := range stakes {
			...
			if _, exists := stakeGroupMap[stake.Source]; !exists {
				...
				effectiveStakes = append(effectiveStakes, []*score.Stake{})
			}
	...
	var srdsr *slst.StakeRewardDistributionRuleSet
...
// func grantEliteEdgeNodeReward(ledger score.Ledger, view *slst.StoreView, guardianVotes *score.AggregatedVotes, eliteEdgeNodeVotes *score.AggregatedEENVotes,
// 	eliteEdgeNodePool score.EliteEdgeNodePool, accountReward *map[string]types.Coins, blockHeight uint64) {
// 	if !common.IsCheckPointHeight(blockHeight) {
// 		return
// 	}
...
func handleSplit(stake *score.Stake, srdsr *slst.StakeRewardDistributionRuleSet, reward *big.Int, accountRewardMap *map[string]types.Coins) {
	if srdsr == nil {
		// Should not happen
...
func issueFixedReward(effectiveStakes [][]*score.Stake, totalStake *big.Int, accountReward *map[string]types.Coins, totalReward *big.Int, srdsr *slst.StakeRewardDistributionRuleSet, rewardType string) {
	if totalStake.Cmp(big.NewInt(0)) == 0 {
		return
	}
...
//func issueRandomizedReward(ledger score.Ledger, guardianVotes *score.AggregatedVotes, view *slst.StoreView, effectiveStakes [][]*score.Stake,
//	totalStake *big.Int, accountReward *map[string]types.Coins, totalReward *big.Int, srdsr *slst.StakeRewardDistributionRuleSet, rewardType string) {
//
//	if guardianVotes == nil {
//		// Should never reach here
//		panic("guardianVotes == nil")
