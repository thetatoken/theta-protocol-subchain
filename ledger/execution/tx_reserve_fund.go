...
func NewReserveFundTxExecutor(state *slst.LedgerState) *ReserveFundTxExecutor {
	return &ReserveFundTxExecutor{
		state: state,
	}
}

func (exec *ReserveFundTxExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
...
func (exec *ReserveFundTxExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	tx := transaction.(*types.ReserveFundTx)
...
func (exec *ReserveFundTxExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := transaction.(*types.ReserveFundTx)
	return &score.TxInfo{
