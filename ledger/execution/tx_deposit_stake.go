...
func NewDepositStakeExecutor(state *slst.LedgerState) *DepositStakeExecutor {
	return &DepositStakeExecutor{
		state: state,
	}
}

func (exec *DepositStakeExecutor) sanityCheck(chainID string, view *slst.StoreView, transaction types.Tx) result.Result {
	...
	if (tx.Purpose == score.StakeForValidator || tx.Purpose == score.StakeForGuardian) && stake.TFuelWei.Cmp(types.Zero) != 0 {
		return result.Error("TFuel has to be zero for validator or guardian stake deposit!").
			WithErrorCode(result.CodeInvalidStake)
	}

	if tx.Purpose == score.StakeForEliteEdgeNode && stake.ThetaWei.Cmp(types.Zero) != 0 {
	...
	if tx.Purpose == score.StakeForValidator && stake.ThetaWei.Cmp(score.MinValidatorStakeDeposit) < 0 {
		return result.Error("Insufficient amount of stake, at least %v ThetaWei is required for each validator deposit", score.MinValidatorStakeDeposit).
			WithErrorCode(result.CodeInsufficientStake)
	}

	//if tx.Purpose == score.StakeForGuardian {
	//	minGuardianStake := score.MinGuardianStakeDeposit
	//	if blockHeight >= common.HeightLowerGNStakeThresholdTo1000 {
	//		minGuardianStake = score.MinGuardianStakeDeposit1000
	//	}
	...
	//if tx.Purpose == score.StakeForEliteEdgeNode {
	...
	//	}
	//
	//	minEliteEdgeNodeStake := score.MinEliteEdgeNodeStakeDeposit
	//	maxEliteEdgeNodeStake := score.MaxEliteEdgeNodeStakeDeposit
	//
...
func (exec *DepositStakeExecutor) process(chainID string, view *slst.StoreView, transaction types.Tx) (common.Hash, result.Result) {
	//blockHeight := view.Height() + 1 // the view points to the parent of the current block
...
//func (exec *DepositStakeExecutor) getEliteEdgeNodeStake(view *slst.StoreView, eenAddr common.Address) *big.Int {
...
func (exec *DepositStakeExecutor) getTxInfo(transaction types.Tx) *score.TxInfo {
	tx := exec.castTx(transaction)
	return &score.TxInfo{
