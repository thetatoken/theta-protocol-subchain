...
func (c *Compiler) compileElement(element token) error {
	...
	if isJump(element.text) {
		...
		c.pushBin(svm.OpCode(int(svm.PUSH1) - 1 + len(value)))
		c.pushBin(value)
	} else {
		c.pushBin(toBinary(element.text))
	}
...
// toBinary converts text to a svm.OpCode
func toBinary(text string) svm.OpCode {
	return svm.StringToOp(strings.ToUpper(text))
}
