...
func loadInitialBalances(erc20SnapshotJSONFilePath string) *slst.StoreView {
...
func performInitialStakeDeposit(stakeDepositFilePath string, genesisHeight uint64, sv *slst.StoreView) *score.ValidatorCandidatePool {
...
func proveVCP(sv *slst.StoreView) (*score.VCPProof, error) {
	vp := &score.VCPProof{}
...
func sanityChecks(sv *slst.StoreView) error {
	...
	sv.GetStore().Traverse(nil, func(key, val common.Bytes) bool {
		if bytes.Compare(key, slst.ValidatorCandidatePoolKey()) == 0 {
			var vcp score.ValidatorCandidatePool
			...
			vcpAnalyzed = true
		} else if bytes.Compare(key, slst.StakeTransactionHeightListKey()) == 0 {
	...
	_, _, err = trie.VerifyProof(sv.Hash(), slst.ValidatorCandidatePoolKey(), vcpProof)
	if err != nil {
