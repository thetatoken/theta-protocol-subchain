...
func getMintlog1(fromBlock, toBlock int, contractAddr common.Address) *common.Address {
	const RawABI = `
[
	{
		"anonymous": false,
	...
	queryStr := fmt.Sprintf(`{"jsonrpc":"2.0","method":"eth_getLogs","params":[{"fromBlock":"%v","toBlock":"%v","topics":["%v"]}],"id":74}`, fmt.Sprintf("%x", fromBlock), fmt.Sprintf("%x", toBlock), crypto.Keccak256Hash([]byte("has(uint256)")).Hex())
	...
	for idx, logData := range rpcres.Result {
		...
		fmt.Println("blockchainid", event.X)
	}
	return nil
}
